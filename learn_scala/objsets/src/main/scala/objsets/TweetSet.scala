...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted = {
    lazy val lmrt = left.mostRetweeted
    lazy val rmrt = right.mostRetweeted

    if( !left.empty && lmrt.retweets > elem.retweets  )
      if( !right.empty && rmrt.retweets > lmrt.retweets )
        rmrt
      else
        lmrt
    else if( !right.empty && rmrt.retweets > elem.retweets )
      rmrt
    else
      elem
  }
